...
from SenNet.network.losses.enhancement_pretrain_losses import EnhancementPretrainLoss
from SenNet.network.net.enhancement_pretrain_net import EnhancementPretrainNet
from SenNet.trainer.trainers import SenTrainer

# 1080Ti 等老卡禁用 torch.compile / inductor / triton
torch._dynamo.config.suppress_errors = True


class EnhancementPretrainTrainer(SenTrainer):
    """
    第三步：增强模块预训练 Trainer

    功能：
    1. 输入原始 CBCT patch；
    2. 网络输出 enhanced / residual；
    3. 只训练增强模块；
    4. 验证阶段不计算 tp/fp/fn/dice；
    5. 按最小 val_loss 保存 checkpoint_best.pth。
    """
    ...
    def __init__(
        ...
        # 不使用 nnUNet 默认 segmentation loss，避免被父类覆盖后误用
        self.enhancement_loss = EnhancementPretrainLoss(
            lambda_rec=1.0,
            lambda_edge=0.2,
            lambda_freq=0.1,
            lambda_res=0.01,
    ...
    def build_network_architecture(
        ...
        network = EnhancementPretrainNet(**architecture_kwargs)
    ...
    def train_step(self, batch: Dict[str, Any]) -> Dict[str, float]:
        ...
        return {
            ...
            "rec": float(loss_dict["rec"].detach().cpu().numpy()),
            "edge": float(loss_dict["edge"].detach().cpu().numpy()),
            "freq": float(loss_dict["freq"].detach().cpu().numpy()),
            "residual_reg": float(loss_dict["residual_reg"].detach().cpu().numpy()),
        }
    ...
    def on_validation_epoch_end(self, val_outputs: List[Dict[str, float]]) -> None:
        ...
        # self.logger.log("val_rec", mean_rec, self.current_epoch)
        # self.logger.log("val_edge", mean_edge, self.current_epoch)
        # self.logger.log("val_freq", mean_freq, self.current_epoch)
        # self.logger.log("val_residual_reg", mean_residual_reg, self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        # if "val_rec" in self.logger.my_fantastic_logging and len(self.logger.my_fantastic_logging["val_rec"]) > 0:
        #     self.print_to_log_file("val_rec", np.round(self.logger.my_fantastic_logging["val_rec"][-1], decimals=4))
        # if "val_edge" in self.logger.my_fantastic_logging and len(self.logger.my_fantastic_logging["val_edge"]) > 0:
        #     self.print_to_log_file("val_edge", np.round(self.logger.my_fantastic_logging["val_edge"][-1], decimals=4))
        # if "val_freq" in self.logger.my_fantastic_logging and len(self.logger.my_fantastic_logging["val_freq"]) > 0:
        #     self.print_to_log_file("val_freq", np.round(self.logger.my_fantastic_logging["val_freq"][-1], decimals=4))
        # if "val_residual_reg" in self.logger.my_fantastic_logging and len(self.logger.my_fantastic_logging["val_residual_reg"]) > 0:
        #     self.print_to_log_file(
        #         "val_residual_reg",
        #         np.round(self.logger.my_fantastic_logging["val_residual_reg"][-1], decimals=4),
        #     )
