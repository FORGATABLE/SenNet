...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.epa_block = EPA(
            ...
            hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        #     self.pos_embed = nn.Parameter(torch.zeros(1, N, C, device=x.device))
...
class EPA(nn.Module):
    ...
    def __init__(self,
                 input_size,
                 hidden_size, proj_size, num_heads=4, qkv_bias=False, channel_attn_drop=0.1, spatial_attn_drop=0.1):
        super().__init__()
    ...
    # def _get_proj_layer(self, N, device):
    #     if N not in self.E_dict:
    #         self.E_dict[N] = nn.Linear(N, self.proj_size).to(device)
    #         self.F_dict[N] = nn.Linear(N, self.proj_size).to(device)
    #     return self.E_dict[N], self.F_dict[N]
    ...
    def forward(self, x):
        ...
        # F = self.F_dict.get(N)
        # if E is None:
        #     E, F = self._get_proj_layer(N, device)
