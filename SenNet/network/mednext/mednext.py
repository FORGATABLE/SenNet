...
class MedNeXtUpBlock(MedNeXtBlock):
    ...
    def forward(self, x, target_shape=None):
        ...
        # x1 = F.interpolate(x1, size=target_shape, mode='trilinear', align_corners=False)
        ...
        if self.resample_do_res:
            ...
            # res = F.interpolate(res, size=target_shape, mode='trilinear', align_corners=False)
            x1 = x1 + res
...
class MedNeXt(nn.Module):
    ...
    def forward(self, x):
        ...
        """
            SenSen:修改尺寸对齐
        """
        x_up_3 = self.up_3(x, target_shape=x_res_3.shape[2:])
        ...
        x_up_2 = self.up_2(x, target_shape=x_res_2.shape[2:])
        ...
        x_up_1 = self.up_1(x, target_shape=x_res_1.shape[2:])
        ...
        x_up_0 = self.up_0(x, target_shape=x_res_0.shape[2:])
        ...
        if self.do_ds:
            '''
            SenSen：Deep Supervision 兼容nnU-Net。
            '''
