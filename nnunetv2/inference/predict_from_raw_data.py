...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        if trainer_class is None:
            """
                        SenSen:修改适配自己的trainer
                    """
            trainer_class = recursive_find_python_class(join(SenNet.__path__[0], "trainer"), trainer_name,
                                                         'SenNet.trainer')
            if trainer_class is None:
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor) -> torch.Tensor:
        ...
        for params in self.list_of_parameters:
            ...
            # if not self._onnx_exported:
            #     self._onnx_exported = True
            #     self.network.cpu()
            #     input_names = ["input"]
            #     output_names = ["output"]
            #     dummy_input = torch.randn(1, 1, 64, 192, 192)
            #     dynamic_axes = {'input': {0: 'batch_size', 2: 'depth', 3: 'height', 4: 'weight'},
            #                     'output': {0: 'batch_size', 2: 'depth', 3: 'height', 4: 'weight'}}
            #     torch.onnx.export(self.network, dummy_input, "/data/zr/onnx_files/CT_Ablation_pre.onnx", export_params=True,
            #                       do_constant_folding=True, verbose=True, input_names=input_names,
            #                       output_names=output_names, dynamic_axes=dynamic_axes)
            #     print("Torch format model to onnx done!")
            #
            #     self.network.cuda()
