...
patch_size = [50, 256, 256]
half_patch_size = [int( x / 2) for x in patch_size]
# 遍历大文件夹中的所有小文件夹，并且把所有小文件夹的名称按顺序存到一个列表中去,得到case_list
import os

data_dir = '/data/zr/Liver_nnUNet_Dataset/nnUNet_raw/Dataset018_LiTS/imagesTr'
label_dir = '/data/zr/Liver_nnUNet_Dataset/nnUNet_raw/Dataset018_LiTS/labelsTr' # 需要使用实例标签

save_datadir = '/data/zr/Liver_nnUNet_Dataset/nnUNet_raw/Dataset020_LiTS/imagesTr/'
save_labeldir = '/data/zr/Liver_nnUNet_Dataset/nnUNet_raw/Dataset020_LiTS/labelsTr/'
...
for case_name in os.listdir(data_dir): # 测试数据
    ...
    for i in unique_values:
        ...
        # x轴
        flag = False
        centroid_x = int(torch.mean(idx_x.float())) # + random.randint(10, 15)
        centroid_y = int(torch.mean(idx_y.float())) # + random.randint(25, 35)
        centroid_z = int(torch.mean(idx_z.float())) # + random.randint(30, 45)
        ...
        if (centroid_y <= mask.shape[1] - half_patch_size[1] and
            ...
            flag = True
            temp = min(centroid_y, 15)
            mask = mask[: ,mask.shape[1] - patch_size[1] - temp:mask.shape[1] - temp,:]
            img = img[: ,img.shape[1] - patch_size[1] - temp:img.shape[1] - temp,:]
            ...
            flag = True
            temp = min(centroid_y, 15)
            mask = mask[: ,temp:patch_size[1] + temp,:]
            img = img[: ,temp:patch_size[1] + temp,:]
        ...
        if (centroid_z <= mask.shape[2] - half_patch_size[2] and
            ...
            temp = min(centroid_z, 15)
            flag = True
            mask = mask[: ,: ,mask.shape[2] - patch_size[2] - temp:mask.shape[2] - temp]
            img = img[: ,: ,img.shape[2] - patch_size[2] - temp:img.shape[2] - temp]
            # mask = mask[:,:,:patch_size]
            # img = img[:,:,:patch_size]
        elif (centroid_z < mask.shape[2] - half_patch_size[2] and
              centroid_z < half_patch_size[2]):
            ...
            temp = min(centroid_z, 15)
            flag = True
            mask = mask[: ,: ,temp:temp + patch_size[2]]
            img = img[: ,: ,temp:temp + patch_size[2]]
        ...
        Mask_path = save_labeldir +case_name + '.nii.gz'
        ...
        CT_path = save_datadir + case_name + '_0000.nii.gz'
